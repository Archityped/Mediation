...
public sealed partial class MediatorTypeInferenceExtensionsGenerator : IIncrementalGenerator
{
    ...
    /// <summary>
    /// Creates a syntax provider that identifies type declarations with base lists.
    /// </summary>
    /// <param name="context">The generator initialization context.</param>
    /// <returns>
    /// An <see cref="IncrementalValuesProvider{T}"/> that provides syntax contexts for type declarations
    /// that have base type or interface implementations.
    /// </returns>
    private static IncrementalValuesProvider<GeneratorSyntaxContext> CreateSyntaxProvider(IncrementalGeneratorInitializationContext context)
        => context.SyntaxProvider.CreateSyntaxProvider(
            predicate: static (s, _) => s is TypeDeclarationSyntax t && t.BaseList is not null,
    ...
    /// <returns>
    /// An <see cref="IncrementalValuesProvider{T}"/> containing syntax contexts, request symbols, and stream type indicators
    /// for types that implement request interfaces.
    /// </returns>
    private static IncrementalValuesProvider<(GeneratorSyntaxContext Context, INamedTypeSymbol RequestSymbol, bool IsStreamType)> CombineCandidates(IncrementalValuesProvider<GeneratorSyntaxContext> syntaxProvider, (IncrementalValueProvider<INamedTypeSymbol?> Request, IncrementalValueProvider<INamedTypeSymbol?> StreamRequest) symbolProviders)
    {
        var requestMethodCandidates = syntaxProvider
            .Combine(symbolProviders.Request)
            .Where(static (pair) => pair.Right is not null)
            .Select(static (pair, _) => (Context: pair.Left, RequestSymbol: pair.Right!, IsStreamType: false));

        var streamRequestMethodCandidates = syntaxProvider
            .Combine(symbolProviders.StreamRequest)
            .Where(static (pair) => pair.Right is not null)
            .Select(static (pair, _) => (Context: pair.Left, RequestSymbol: pair.Right!, IsStreamType: true));
    ...
    /// </returns>
    private static IncrementalValuesProvider<RequestInfo> ExtractRequestInfo(IncrementalValuesProvider<(GeneratorSyntaxContext Context, INamedTypeSymbol RequestSymbol, bool IsStreamType)> candidates)
        => candidates
            .Select(static (candidate, _) => GetRequestInfo(candidate.Context, candidate.RequestSymbol, candidate.IsStreamType))
    ...
    private static void RegisterSourceOutput(IncrementalGeneratorInitializationContext context, IncrementalValuesProvider<RequestInfo> requestInfo, IncrementalValueProvider<string?> version)
    {
        ...
        {
            var (requestInfos, version) = items;
            Version = version;

            var grouped = requestInfos
                .Distinct()
            ...
            foreach (var kvp in grouped)
            {
                var ns = kvp.Key;
                var pairs = kvp.Value;
                var fileName = $"{Sanitize(ns.AsSpan())}.{ClassName}.g.cs";
    ...
    /// <summary>
    /// Extracts request information from a syntax context and request symbol.
    /// </summary>
    /// <param name="context">The generator syntax context containing the type declaration.</param>
    ...
    /// </returns>
    /// <remarks>
    /// This method validates that the type is concrete (not abstract) and is either a class or struct
    /// before extracting the request and response type information.
    /// </remarks>
    private static RequestInfo? GetRequestInfo(GeneratorSyntaxContext context, INamedTypeSymbol requestSymbol, bool IsStreamType)
    {
        ...
        foreach (var interfaceSymbol in typeSymbol.AllInterfaces)
        {
            ...
            var targetNamespace = requestType.ContainingNamespace?.ToDisplayString();
    ...
    private static string GenerateNamespaceExtensionFile(string targetNamespace, IEnumerable<(ITypeSymbol TRequest, ITypeSymbol TResponse, bool IsStreamType)> pairs)
    {
        ...
        void AddNamespaceIfValid(INamespaceSymbol? namespaceSymbol)
        {
            if (namespaceSymbol?.ToDisplayString() is { } ns && !string.IsNullOrWhiteSpace(ns))
                usings.Add(ns);
        }
        ...
        {
            var requestTypeName = tRequest.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);
            var responseTypeName = tResponse.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);

            AddNamespaceIfValid(tRequest.ContainingNamespace);
            AddNamespaceIfValid(tResponse.ContainingNamespace);

            if (tResponse is INamedTypeSymbol named && named.IsGenericType)
            {
                foreach (var arg in named.TypeArguments)
                    AddNamespaceIfValid(arg.ContainingNamespace);
            }
            ...
            var method = CreateMethodDeclaration(requestTypeName, responseTypeName, IsStreamType);
            methods.Add(method);
        }

        return GenerateCompilationUnit(targetNamespace, usings, methods);
    }
    ...
    /// </remarks>
    private static string GenerateCompilationUnit(string targetNamespace, IEnumerable<string> usings, IEnumerable<MemberDeclarationSyntax> methods)
    {
        var classDeclaration = CreateClassDeclaration(methods);
